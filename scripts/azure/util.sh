...
createAKSCluster() {
    ...
    then
        ...
        az aks create --resource-group ${DEPLOYMENT_NAME} --name ${AZ_CLUSTER_NAME} --node-count ${AZ_NODE_COUNT} --node-vm-size ${AZ_NODE_SIZE} --generate-ssh-keys -l "${AZ_LOCATION}" --service-principal ${AZ_CLIENT_ID} --client-secret "${AZ_CLIENT_SECRET}"
...
createAzSecrets() {
  ...
  # create the namespace for the secret
  createNamespace
